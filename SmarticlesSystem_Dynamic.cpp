...
retry:
	...
	switch (bucketType)
	{
		case HOPPER:
		{
			...
			zposi = (-yposi - 2 * sys->bucket_half_thick)*tan(Quat_to_Angle(AngleSet::RXYZ, sys->bucket->GetRot()).x()) + t_smarticle / 1.99; //tangent may need to be fixed see buckrotx above
			break;
		}
	...
	if (mySmarticlesVec.size() > 0)// more than 1 smarticle exists in system 
	{
		for (int k = 0; k < mySmarticlesVec.size(); k++)//I think I want to go loop through all but current smarticle 
		{
			...
			//cs->RotateSmarticleBy(Angle_to_Quat(AngleSet::RXYZ, ChVector<>(Quat_to_Angle(AngleSet::RXYZ, bucket->GetRot()).x(), /*genRand(0, 2*PI)*/ 0, PI/2)));
			cs->SetEdges();
	...
	else//only 1 smarticle exists
	{
		...
		//cs->RotateSmarticleBy(Angle_to_Quat(AngleSet::RXYZ, ChVector<>(PI / 2 + Quat_to_Angle(AngleSet::RXYZ, bucket->GetRot()).x(), genRand(-PI, PI), 0)));
		cs->SetEdges();
...
void AddParticlesLayer1(std::shared_ptr<CH_SYSTEM> mphysicalSystem, std::vector<std::shared_ptr<Smarticle>> & mySmarticlesVec,double timeForDisp) {
	...
	for (int i = 0; i < numPerLayer; i++)
	{
		...
		case HOPPER:
			...
			myRot = Angle_to_Quat(AngleSet::RXYZ, ChVector<>(genRand(-PI, PI), genRand(-PI, PI), genRand(-PI, PI)));
			break;
		case BOX: case FLATHOPPER:
		{
			...
			//double buckRotAngx = Quat_to_Angle(AngleSet::RXYZ, buckRot).x();
			////myRot = buckRot*Angle_to_Quat(AngleSet::RXYZ,ChVector<>(PI/2,PI,0));
			////myRot = buckRot*Angle_to_Quat(AngleSet::RXYZ, ChVector<>(genRand(0, 2*PI), genRand(0, 2*PI), genRand(0, 2*PI)));

			////ypos genRand(-PI,PI)

			//myRot = buckRot*Angle_to_Quat(AngleSet::RXYZ, ChVector<>(PI / 2, genRand(-PI, PI), 0));
			...
			////////myRot = buckRot*Angle_to_Quat(AngleSet::RXYZ, ChVector<>(PI / 2, PI / 2, 0));
...
{	
	...
	utils::AddCylinderGeometry(xaxis.get(), rad, len, ChVector<>(len - rad / 2, 0, 0) + pos, Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, PI_2)), true);//bottom
	utils::AddCylinderGeometry(yaxis.get(), rad, len, ChVector<>(0, len-rad/2, 0) + pos, Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, 0)), true);//bottom, true);//bottom
	utils::AddCylinderGeometry(zaxis.get(), rad, len, ChVector<>(0, 0, len - rad) + pos, Angle_to_Quat(AngleSet::RXYZ, ChVector<>(PI_2, 0, 0)), true);//bottom
...
//  for (int tStep = 0; tStep < 1; tStep++) {
	...
	for (int tStep = 0; tStep < stepEnd + 1; tStep++) {
		...
		{
			sys->bucket_bott->SetBodyFixed(true);
			sys->rotate_body_rot(t, sys->bucket, sys->bucket_actuator, Quat_to_Angle(AngleSet::RXYZ, sys->bucket->GetRot()).x());
		}
		//vibration movement
