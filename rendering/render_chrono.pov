...
#declare datafile = concat("/home/pazouki/chronoStuff/runsFSI/run264/PostProcess/povFilesSmarticles/data_", str(fnum,-3,0), ".dat")
//#declare datafile = concat("/home/pazouki/chronoStuff/runsFSI/run121/PostProcess/povFilesSmarticles/data_", str(fnum,-3,0), ".dat")
//#declare datafile = concat("/home/arman/programBuilts/smarticlesBuild/PostProcess/povFilesSmarticles/data_", str(fnum,-3,0), ".dat")
//#declare fluid_file1 = concat("/home/pazouki/chronoStuff/runsFSI/run121/PostProcess/povFilesFluid/fluid", str(fnum-1,-1,0), ".csv")
//#declare fluid_boundary_file = concat("/home/pazouki/chronoStuff/runsFSI/run121/PostProcess/povFilesFluid/fluid_boundary", str(fnum-1,-1,0), ".csv")
...
#declare cam_loc    = <-.300, -.200, .200>;
#declare cam_lookat = <0, 0, -.010>;

/*
#declare cam_perspective = true;
#declare cam_loc    = <-200, -100, 0>;
#declare cam_lookat = <0, 0, -50>;
*/

//good one
//#declare cam_perspective = true;
//#declare cam_loc    = <-4, -8, 2>;
...
// --------------------------------------------------------------------------------------------   
// Create a temporary file to parse the string into it. The name of the tmp file is specific 
// to each scene. This is useful when rendering multiple scenes in parallel. In such scenarios,
// the built-in Parse_String function is not usefull since all scenes write/read to/from the 
// same file called parse_string.tmp. The obly problem with this new function is that it 
// creates as many files as the number of scenes in the work directory.         
// parse string macro which parses each time from a different file
// NOTE: when using Parse_String or Custom_ParseString, do not use multi thread to render a 
// single scene. Apparently it causes error since you are calling this function multiple time 
// per scene and that would confuse the input/output .

#macro Custom_ParseString(S)
     #local Filename = 
concat("tmp/so_parsestring",str(fnum,0,0),".tmp");
...
#local mesh_include = concat(mesh_name,".inc");
#include mesh_include
  //  Parse_String(concat("#include \"", mesh_name,"\""))
//    Parse_String(concat(mesh_name, " position(pos,rot)"))
Custom_ParseString(concat(mesh_name, " position(pos,rot)"))
//#declare mesh_cmd = concat(mesh_name, " position(pos,rot)");
//mesh_cmd
#end   
...
texture { Glass }
		...
		#local strMeshName = mesh_name;
		#local subMeshName = substr(strMeshName, 1, 3);
...
// ============================================================================================   
/*
#fopen MyPosFile1 fluid_file1 read
#fopen MyPosFile4 fluid_boundary_file read
#declare px=0.0;
#declare py=0.0;
#declare pz=0.0;
#declare vx=0.0;
...
#while(defined(MyPosFile1))
	#read (MyPosFile1, px, pz, py, vx,vy,vz, vv, e0,e1, e2)
	#local c=<1,1,1>;
	#local p=vv/5;
	#local max_c=1;
	#if (p <= .5)
	...
	union{
		box{<.4,-.2,-.2>, <1.5,.7,1.1>}
		box{<1,-.2,-.2>, <1.5,1.5,1.1>}

		box{<-.1,-.1,-.2>, <1.5,.015,1.1>}
		box{<-.1,-.1,.98>, <1.5,1.1,1.1>}
		box{<-.1,-.1,-.1>, <.015,1.1,1.1>}
		box{<-.1,.98,-.1>, <1.5,1.1,1.1>}
	}

	pigment {color  rgbt <c.x,c.y,c.z,0> }
	finish {diffuse 1 ambient 0 specular .00 } 
	}
