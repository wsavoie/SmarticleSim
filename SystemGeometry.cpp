...
std::shared_ptr<ChBody> SystemGeometry::create_ChordRing(int num_boxes, double half_h, double t, double r,double sagitta, ChVector<> pos, std::shared_ptr<ChTexture> texture, double m)
{
	...
	quat = Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, boxChordAng));
	...
	quat = Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, 2*PPI-boxChordAng-PPI/2));
...
std::shared_ptr<ChBody> SystemGeometry::create_FlatHopper(ChVector<> hdim)
{
	...
	utils::AddBoxGeometry(flathopper.get(), ChVector<>(hthick, hopperWallHeight, hdim.z() + o_lap), //left side
		ChVector<>(hdim.x() - hopperWallWidth, -hdim.y() - hopperWallHeight*cos(wallAngle), hdim.z()), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, wallAngle)));

	utils::AddBoxGeometry(flathopper.get(), ChVector<>(hthick, hopperWallHeight, hdim.z() + o_lap), //right side
		ChVector<>(-(hdim.x() - hopperWallWidth), -hdim.y() - hopperWallAngleHeight, hdim.z()), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, -wallAngle)));
	flathopper->GetCollisionModel()->BuildModel();

	flathopper->SetRot(Angle_to_Quat(AngleSet::RXYZ, ChVector<>(box_ang, 0, 0)));
	...
	bucket_bott->SetRot(Angle_to_Quat(AngleSet::RXYZ, ChVector<>(box_ang, 0, 0)));
...
std::shared_ptr<ChBody> SystemGeometry::create_Hopper(double theta, bool overlap)
{
	...
	utils::AddBoxGeometry(hopper.get(), ChVector<>(t, r + t, h), hop_pos - VECT_X*(-sin(ang)*h + hole_size + cos(ang)*t) + VECT_Z*(h - o_lap), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, -ang, 0)), true);// -x negAngle
	//utils::AddBoxGeometry(hopper.get(), ChVector<>(t, r + t, h), hop_pos - VECT_X*(sin(ang)*(h)+hole_size+cos(ang)*t) + VECT_Z*(h - o_lap), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, ang, 0)), true);// -x negAngle +theta
	utils::AddBoxGeometry(hopper.get(), ChVector<>(t, r + t, h), hop_pos + VECT_X*(-sin(ang)*h + hole_size + cos(ang)*t) + VECT_Z*(h - o_lap), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, ang, 0)), true);// +x  -theta = /
...
std::shared_ptr<ChBody> SystemGeometry::create_Hull(double numBoxes)
{
	...
	//utils::AddBoxGeometry(convexShape.get(), rampSize, rampPos, Angle_to_Quat(AngleSet::RXYZ, ChVector<>(box_ang, 0, 0)), true);
...
void SystemGeometry::create_Truss()
{
	...
	utils::AddCylinderGeometry(truss.get(), t2_smarticle / 2, bucket_interior_halfDim.z() * 1, bucket_ctr + ChVector<>(0, 0, bucket_interior_halfDim.z()), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(-PI_2, 0, 0)), false);
...
void SystemGeometry::create_Knobs(double kpr,double rows, double length)
{
	...
	//utils::AddCylinderGeometry(truss.get(), t2_smarticle / 2, bucket_interior_halfDim.z() * 1, bucket_ctr + ChVector<>(0, 0, bucket_interior_halfDim.z()), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(PI_2, 0, 0)), true);
	...
	for (size_t row = 0; row < rows; row++)
	{
		for (size_t col = 0; col < kpr; col++)
		{
			...
			utils::AddBoxGeometry(stick.get(), ChVector<>(knobRad*1.5, rad / 4, rad / 8), bucket_ctr + ChVector<>(rad*cos(theta), rad*sin(theta), hp*(row + 1)), Angle_to_Quat(AngleSet::RXYZ, ChVector<>(0, 0, theta + row % 2 * PI_2)), true);
...
void SystemGeometry::create_CentralColumn(double length)
{
	...
	for (size_t i = 0; i < sphereNum; i++)
	{
		...
		utils::AddSphereGeometry(stick.get(), rad, bucket_ctr + ChVector<>(0, 0, stickLen / sphereNum * (i)), Angle_to_Quat(AngleSet::RXYZ, ChVector<double>(0, 0, PPI)), true);
		sphereStick.emplace_back(stick);
	}
	...
	if (bucketType == HOOKRAISE)
	{
		...
		for (size_t i = 0; i < hookNum; i++)
		{
			if (stapleSize)
			{
				...
				//AddBoxGeometry
				utils::AddSphereGeometry(stick.get(), rad, bucket_ctr + ChVector<>(rad*(i + 1), 0, stickLen / sphereNum), Angle_to_Quat(AngleSet::RXYZ, ChVector<double>(PPI, 0, 0)), true);
