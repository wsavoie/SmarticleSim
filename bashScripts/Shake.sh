...
for i in `seq 0 0`; do
  echo $i
  a=./$foldName/${lwArr[$i]}-${nl[$i]}-${npl[$i]}/${vers}
  cd $a
  $smartRunFile ${lwArr[$i]} ${dtVal} ${nl[$i]} ${re} ${pa} ${ang1} ${ang2} 0 ${npl[$i]} 0.2 0 0 0 0 1;
  #%SMARTDIR% %LW% %DT% %NUMLAYER% %READFILE% %ACTIVEPCT% %ANGLE1% %ANGLE2% %BOXANG% %NUMPERLAYER% %STRESSACTIVATE% %SCREENSHOT% %RINGPART% %WINDX% %WINDY% %SMARTINACTIVE%
  cd $mf;
  # cp -r ./PostProcess $a
done;