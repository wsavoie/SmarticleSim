	...
	if (event.EventType == irr::EET_KEY_INPUT_EVENT && !event.KeyInput.PressedDown)
	{
		...
		case irr::KEY_KEY_1:
			...
			case FLATHOPPER: case BOX:
				...
				//rampAngle = Quat_to_Angle(ANGLE, bucket->GetRot()).x() - rampInc * D2R;
				//bucket->SetRot(Angle_to_Quat(ANGLE, ChVector<>(
				//	Quat_to_Angle(ANGLE, bucket->GetRot()).x() - rampInc * D2R
				//	, 0, 0)));
				//bucket_bott->SetRot(Angle_to_Quat(ANGLE, ChVector<>(
				//	Quat_to_Angle(ANGLE, bucket->GetRot()).x() - rampInc * D2R
				//	, 0, 0)));
				break;
			default:
				box_ang = Quat_to_Angle(ANGLE, sys->bucket->GetRot()).x() - rampInc * D2R;
				sys->bucket->SetRot(Angle_to_Quat(ANGLE, ChVector<>(
					Quat_to_Angle(ANGLE, sys->bucket->GetRot()).x() - rampInc * D2R
					, 0, 0)));
				break;
			}
			drawAngle();
...
void IrrGui::drawAngle()
{
	...
	else if (bucketType == BOX)
	{
		...
		char message[100]; sprintf(message, "Angle: %1.3g, Increment: %1.3g", Quat_to_Angle(ANGLE, sys->bucket->GetRot()).x() * R2D, rampInc);
		this->text_Angle->setText(core::stringw(message).c_str());
	}
