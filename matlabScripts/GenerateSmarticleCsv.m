...
ss=radI*alen*(pi/180); %rad*degs*degToRads   this is arc length each step
...
switch global_gait
    ...
    case 5
        ...
        axis square
        figText(gcf,15);
    case 6% positive square gait
        %not implemented yet!
        sL = pi; %square gait side length
        len = length(0:ss:sL); %gait length
        sL2 = pi/2;%*.8;
        %theta1
        amove1 = linspace(-sL/2,sL/2,len);    %0   ->  max
        amove2 = linspace(sL/2,sL/2,len);   %max ->  max
        amove3 = linspace(sL/2,-sL/2,len);    %max ->  0
        amove4 = linspace(-sL/2,-sL/2,len);     %0   ->  0
        %       theta2
        bmove1 = linspace(-sL2/2,-sL2/2,len);     %0   ->  0
        bmove2 = linspace(-sL2/2,sL2/2,len);   %0   ->  max
        bmove3 = linspace(sL2/2,sL2/2,len); %max ->  max
        bmove4 = linspace(sL2/2,-sL2/2,len);   %max ->  0
        %         global_alpha_1Pos=[t1,l1,b1,r1];
        %         global_alpha_2Pos=[t2,l2,b2,r2];
        global_alpha_1Pos=[amove1,amove2,amove3,amove4];
        global_alpha_2Pos=[bmove1,bmove2,bmove3,bmove4];
